...
class CandlestickCalculator:
    def __init__(
        ...
        self.data = data
        self.key_levels = sorted(key_levels, key=lambda level: -level.level)
        self.key_level = None

    def do(self, trend_type: TrendTypes) -> Tuple[int, KeyLevel]:
        point = self.get(CandlestickPatterns.MOMENTUM_CANDLE, trend_type)
        if point <= 0:
            return 0, KeyLevel(0, 0, 0)
        # debug_text()
    ...
    def side(self, level: float) -> int:
        ...
        for candle in self.data[-Config.get('key-level.breakthrough-window'):]:
            cur = +1 if candle.highest > level else -1
            if sign == 0:
                sign = cur
            if sign != cur:
                return 0
    ...
    def long_wick_candle(self, trend_type: TrendTypes) -> int:
        bl = True
        if trend_type is TrendTypes.UP:
            bl = False
            ...
            for i in range(Config.get('key-level.long-wick-backward')):
                ...
                # gk &= not self.__cross_down(self.data[index + 1].closing)[0]
